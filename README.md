...
## Работа с удаленными репозиториями
  * __git clone__ - клонирует удаленный репозиторий на компьютер.
  * __git push__ - отправляет локальную версию репозитория на удаленный. Сервис может запросить авторизацию.
  * __git pull__ - забирает изменения из удаленного репозитория, а затем пытается ее слить с текущей веткой.
## Запрос __pull request__
  1. Совершаем Fork репозитория
  2. Клонируем (git clone) репозиторий у себя.
  3. Создаем новую ветку и вносим изменения.
  4. Фиксируем изменения (git commit)
  5. Отправляем свою версию (git push) в свой GitHub
  6. Отправляем запрос (pull request)